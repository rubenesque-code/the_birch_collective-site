...
export type StaticData = {
  ...
  page: ReturnType<(typeof processDbData)["donatePage"]["crossProcess"]>;
};
...
export const getStaticProps: GetStaticProps<StaticData> = async () => {
  ...
  const page = await myDb.pages.donate.fetch();
  ...
  const pageCrossProcessed = processDbData.donatePage.crossProcess(page, {
