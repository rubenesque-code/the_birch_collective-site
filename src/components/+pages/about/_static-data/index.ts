...
export type StaticData = {
  ...
  page: ReturnType<(typeof processDbData)["aboutPage"]["crossProcess"]>;
};
...
export const getStaticProps: GetStaticProps<StaticData> = async () => {
  ...
  const page = await myDb.pages.aboutUs.fetch();

  const connectedImageIds = [
    orgDetails.logoImage.dbConnections.imageId,

    page.bannerImage.dbConnections.imageId,
    ...page.theTeam.members.map((entry) => entry.image.dbConnections.imageId),
  ].flatMap((i) => (i ? [i] : []));

  const connectedImages = await myDb.image.fetchMany(connectedImageIds);

  const pageCrossProcessed = processDbData.aboutPage.crossProcess(page, {
